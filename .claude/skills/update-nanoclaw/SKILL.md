...
## Branch families on the fork: `skill/*` vs `*-upstream`

Some forks carry **two parallel branch families** for each skill:

- **`skill/<name>`** — full-history development branch. PR merges, back-merges
  from `main`, iterative commits. **This is what the installation merges.** When
  this skill says "skill branches", it means these.
- **`<name>-upstream`** — a **single squashed commit** on top of `main`, shaped
  for contribution PRs to the public upstream (`qwibitai/nanoclaw`). Never merge
  `main` in, never add commits, never merge into the installation. Rebuild by
  re-squashing `skill/<name>` when needed.

If you are not sure which family a branch belongs to, check the installation's
`AGENTS.md` → "Branch conventions on the fork" before touching it. The
`*-upstream` family may be protected by a GitHub ruleset that blocks non-linear
history and force-pushes from non-admins.
