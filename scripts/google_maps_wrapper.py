...
# Credential file names per account
_CREDS_BASENAMES = {
    "1": "google-gmail.json",
    "2": "google-gmail-2.json",
}
...
def _resolve_creds_file(account: str) -> str:
    ...
    Search order:
      1. GOOGLE_CREDS_FILE env var (account 1 only)
      2. ~/.config/nanoclaw/secrets/<basename>
      3. ~/.openclaw/secrets/<basename>
    """
    basename = _CREDS_BASENAMES.get(account, _CREDS_BASENAMES["1"])

    # Env var override (account 1 only)
    if account == "1":
        env_path = os.environ.get("GOOGLE_CREDS_FILE")
        if env_path:
            expanded = os.path.expanduser(env_path)
            if os.path.exists(expanded):
                return expanded

    # ~/.config/nanoclaw/secrets/
    nanoclaw_path = os.path.expanduser(f"~/.config/nanoclaw/secrets/{basename}")
    if os.path.exists(nanoclaw_path):
        return nanoclaw_path

    # ~/.openclaw/secrets/ (legacy fallback)
    openclaw_path = os.path.expanduser(f"~/.openclaw/secrets/{basename}")
